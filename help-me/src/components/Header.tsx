...
function Navbar() {
    ...
    return(
        <nav className="flex flex-row flex-3 gap-10 text-lg font-medium">
            <Link to="/" className={`flex items-center gap-2 transition-all duration-300 hover:text-accent-700 ${page === "" ? "text-accent-700" : ""}`}>
                <Icon icon="majesticons:home-simple-line" className="w-6 h-6" />
                <span>Home</span>
            </Link>
            <Link to="/questions" className={`flex items-center gap-2 transition-all duration-300 hover:text-accent-700 ${page === "questions" ? "text-accent-700" : ""}`}>
                <Icon icon="majesticons:question-mark-circle-line" className="w-6 h-6" />
                <span>Questions</span>
            </Link>
            <Link to="/faq" className={`flex items-center gap-2 transition-all duration-300 hover:text-accent-700 ${page === "faq" ? "text-accent-700" : ""}`}>
...
function MobileNavbar() {
    ...
    return(
        <nav className="flex flex-col flex-3 gap-2 text-lg font-medium">
            <Link to="/" className={`flex items-center gap-2 transition-all duration-300 active:text-accent-700 ${page === "" ? "text-accent-700" : ""}`}>
                <Icon icon="majesticons:home-simple-line" className="w-6 h-6" />
                <span>Home</span>
            </Link>
            <Link to="/questions" className={`flex items-center gap-2 transition-all duration-300 active:text-accent-700 ${page === "questions" ? "text-accent-700" : ""}`}>
                <Icon icon="majesticons:question-mark-circle-line" className="w-6 h-6" />
                <span>Questions</span>
            </Link>
            <Link to="/faq" className={`flex items-center gap-2 transition-all duration-300 active:text-accent-700 ${page === "faq" ? "text-accent-700" : ""}`}>
                <Icon icon="majesticons:messages-line" className="w-6 h-6" />
                <span>FAQ</span>
            </Link>
        </nav>
...
function ThemeToggler() {
    ...
    return(
        ...
        >
            { theme === "light" ?
                <Icon icon="majesticons:moon" className="w-8 h-8 transition-all duration-300 hover:text-accent-700" />
                :
                <Icon icon="majesticons:sun" className="w-8 h-8 transition-all duration-300 hover:text-accent-700" />   
            }
...
function AccountAccessor() {
    ...
    if(!logedIn) return(
        ...
        >
            <Icon icon="majesticons:user-line" className="w-8 h-8 transition-all duration-300 hover:text-accent-700" />
            <DropdownMenu   
                isOpen={isOpen}
                setIsOpen={setIsOpen}
                togglerRef={btnRef}
        ...
        <div className="cursor-default w-[250px] h-fit bg-background-50 flex flex-col justify-center items-center p-3 border rounded-2xl border-text-200">
            <div className="w-full h-fit my-2 flex flex-row justify-center items-center">
                ...
                <span className="h-full w-full font-medium flex flex-col justify-center items-start ml-5">
                    ...
                    <Link to="/profile/edit" className="flex items-center gap-2 text-text-950 transition-all duration-300 hover:text-accent-700">
            ...
            <div className="w-full h-fit text-lg font-semibold text-text-950 flex flex-col gap-3">
                <Link to="/profile/favorites" className="flex gap-2 text-text-950 items-center transition-all duration-300 hover:text-accent-700">
                    <Icon icon="majesticons:heart-line" className="w-8 h-8" />
                    Favorites
                </Link>
                <Link to="/profile/history" className="flex gap-2 text-text-950 items-center transition-all duration-300 hover:text-accent-700">
                    <Icon icon="majesticons:clock-line" className="w-8 h-8" />
                    History
                </Link>
                <Link to="/profile/notifications" className="flex gap-2 text-text-950 items-center transition-all duration-300 hover:text-accent-700">
                    <Icon icon="majesticons:bell-line" className="w-8 h-8" />
                    Notifications
                </Link>
                <span onClick={() => Logout()} className="flex gap-2 text-text-950 items-center transition-all duration-300 hover:text-accent-700">
